...
for agents in agent_list:
    ...
    elif in_mmo_agent_name == agents['agentName']:
        in_mmo_agent_id = agents['agentId']

    elif no_mmo_agent_name == agents['agentName']:
        no_mmo_agent_id = agents['agentId']
...
for test in destination_list_of_dictionaries:
    ...
    no_mmo_test_name = 'PF: No MMO Test from branch to: ' + str(test['agent_name'])
    in_mmo_test_name = 'PF: In MMO Test from branch to: ' + str(test['agent_name'])

    # crafting body for HTTP Post to create test in TE dashboard for East US Agent
    create_no_mmo_test_data = { "interval": 300,
        "agents": [
          {"agentId": no_mmo_agent_id} 
        ],
        "testName": no_mmo_test_name,
    ...
    create_in_mmo_test_data = { "interval": 300,
        "agents": [
          {"agentId": in_mmo_agent_id} 
        ],
        "testName": in_mmo_test_name,
        "targetAgentId": test['agent_id'],
        "port": 49153,
        "alertsEnabled": 0,
      }
    ...
    create_in_mmo_test = create_agent_to_agent_test(email=your_email, api_token=your_apikey, \
    payload=your_optional_payload, body = json.dumps(create_in_mmo_test_data)) 

    create_no_mmo_test = create_agent_to_agent_test(email=your_email, api_token=your_apikey, \
    payload=your_optional_payload, body = json.dumps(create_no_mmo_test_data)) 

    print(create_in_mmo_test)
    print(create_no_mmo_test)
